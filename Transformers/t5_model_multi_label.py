...
# T5 does not use token_type_ids
...
# T5 for multi-label sequence classification using T5ForSequenceClassification
class T5Class(torch.nn.Module):
    def __init__(self):
        super(T5Class, self).__init__()
        self.t5 = transformers.T5ForSequenceClassification.from_pretrained(
            'google-t5/t5-base', num_labels=labels
        )

    def forward(self, ids, mask):
        logits = self.t5(ids, attention_mask=mask).logits
        return logits
